...
def load_mnist(batch_size=64): # load_mnist라는 이름의 함수를 정의하고, batch_size 매개변수는 데이터를 얼마나 많은 단위로 나눌지 결정
    ...
    builtins.train_loader = torch.utils.data.DataLoader(train_data, batch_size=batch_size) # 데이터셋을 데이터 로더에 로드합니다. 데이터 로더는 데이터셋을 지정된 배치 크기에 맞게 나누고, 이를 반복 가능한 객체로 만들어 학습 과정에서 쉽게 사용할 수 있게 도움
...
def plot_results(hist): # plot_results라는 함수를 정의하는데 hist라는 이름의 딕셔너리를 매개변수로 받는데 학습과 검증 과정의 정확도와 손실이 배열 형태로 저장되어 있음
    ...
    plt.plot(hist['test_acc'], label='Testing acc') # hist 딕셔너리에서 검증 정확도(val_acc)를 추출하여 그래프로 그리는데 라벨을 'Validation acc'로 지정
